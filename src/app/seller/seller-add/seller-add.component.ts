...
export class SellerAddComponent implements OnInit {
  ...
  ngOnInit(): void {
    let sellerId = JSON.parse(localStorage.getItem('sellerLoginStorage'))[
      '_id'
    ];
  ...
  applySub() {
    this.categoryService
      .getAllSubCategoriesOfACategryById(this.product.productCategory)
      .subscribe(
        (res) => {
  ...
  submitAdd(form) {
    var shortName = this.product.productName.replace(/\s/g, '').trim();
    ...
    if (this.editMode) {
      ...
      if (form.value.prodSale == 0) {
        this.product.productPrice.finalPrice =
      ...
      for (let i = 0; i < this.arrayOfImages.length; i++) {
        ...
        if (
          ...
          this.imageService
            .upload(
              img,
              this.product.productSales + shortName + index + Date.now()
            )
            .subscribe(
              (res: any) => {
      ...
      if (this.arrayOfImages.length > this.product.productImages.length) {
        let index = 0;
        for (
          let i = this.product.productImages.length;
          i < this.arrayOfImages.length;
          i++
        ) {
          let img: any = this.arrayOfImages[i];
          console.log(img);
